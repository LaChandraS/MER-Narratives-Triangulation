...
```{r}
narratives_df <- read_excel("Narratives FY2020 Q4 Pre-Cleaning.xlsx", 
    skip = 6)
```
...
# Create ICPI Covid list before running this code
tf_idf_calc <- narratives_df %>%
  ...
  group_by(`Operating Unit`) %>%
  summarise(total = sum(total), covid_n = sum(covid_n), covid_tf = sum(covid_tf), covid_idf = sum(covid_idf), covid_tfidf = mean(covid_tfidf))
...
liwc_summary <- narratives_df %>%
  ...
  group_by(`Operating Unit`) %>%
  summarise(final_col = mean(covid_per))
