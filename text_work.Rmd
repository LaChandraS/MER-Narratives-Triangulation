...
```{r}
Narratives_FY2020_Q3_Pre_Cleaning <- read.csv(file = 'LIWC2015 Q3 MER Narratives COVID HRH Dispense GBV Condoms Words Results Pre-Cleaning.csv', stringsAsFactors = FALSE)
names(Narratives_FY2020_Q3_Pre_Cleaning) <- gsub("\\.", " ", names(Narratives_FY2020_Q3_Pre_Cleaning))
...
```{r}
num_narratives = nrow(Narratives_FY2020_Q3_Pre_Cleaning)

tf_idf_calc <- Narratives_FY2020_Q3_Pre_Cleaning %>%
  mutate(row_num = 1:n()) %>%
  unnest_tokens(word, Narrative) %>%
  anti_join(stop_words) %>%
